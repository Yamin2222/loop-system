...
export function runPlanOnce(root, target, ts = tsNow()) {
...
export function runExecuteOnce(root, target) {
  const planPath = join(root, '.loop', 'plan.md');
  if (!nonEmptyFile(planPath)) {
  ...
  const { status, stdout } = runCoco(promptExecute(target), FIX_TOOLS, { capture: true });
...
export function runVerifyFixOnce(root, target) {
  ...
  const { status, stdout } = runCoco(promptVerifyFix(target), READONLY_TOOLS, { capture: true });
...
export function run(args) {
  ...
  if (mode === 'plan') {
    ...
    const result = runPlanOnce(root, target, ts);
  ...
  if (mode === 'execute') {
    ...
    const result = runExecuteOnce(root, target);
    return result.rc;
  }

  if (mode === 'verify-fix') {
    if (!target) { console.error('用法: loop-system run verify-fix "<目标描述>"'); return 1; }
    const result = runVerifyFixOnce(root, target);
    return result.rc;
  }
  ...
  console.error(`未知模式: ${mode}（支持: triage | plan | execute | verify-fix | fix）`);
  return 1;
}
